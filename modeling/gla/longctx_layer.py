...
class LongctxGatedLinearAttention(nn.Module):
    r"""
    ...
    def __init__(
        ...
        decay_mode: str = 'per_head',
        clamp_min: Optional[float] = None,
        fuse_norm: bool = True,
        layer_idx: int = None,
    ) -> LongctxGatedLinearAttention:
        super().__init__()
        ...
        if decay_mode == 'per_head':
            self.a_proj = nn.Linear(hidden_size, self.num_heads, bias=False)
        elif decay_mode == 'per_channel':
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if self.decay_mode == 'per_head':
            gk = gk.repeat(1, 1, k.size(-1))
    ...
    def get_decay(
        ...
        gk = -self.A_log.float().exp() * F.softplus(self.a_proj(hidden_states).float() + self.dt_bias)
        if self.decay_mode == 'per_head': gk = gk.repeat(1, 1, self.num_heads * self.head_k_dim)
